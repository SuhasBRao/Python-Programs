...
class Solution:
    ...
    def printBoundaryLeft(self, node):
        if (node):
            if node.left:
                self.boundary_nodes.append(node.data)
                
                self.printBoundaryLeft(node.left)

            elif node.right:
                self.boundary_nodes.append(node.data)

                self.printBoundaryLeft(node.right)
    ...
    def printBoundaryRight(self, node):
        
        if node:
            if node.right:
                
                self.printBoundaryRight(node.right)
                self.boundary_nodes.append(node.data)

            elif node.left:
                
                self.printBoundaryRight(node.left)
                self.boundary_nodes.append(node.data)
    ...
    def printBoundaryView(self, root):
        ...
        if (root):
            self.boundary_nodes.append(root.data)
